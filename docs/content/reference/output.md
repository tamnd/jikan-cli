...
describes what users get. Pick a format with `-o`, or let jikan choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
jikan <command> -o table   # aligned columns for reading
jikan <command> -o jsonl   # one JSON object per line, for piping
jikan <command> -o json    # a single JSON array
jikan <command> -o csv     # spreadsheet friendly
jikan <command> -o tsv     # tab-separated
jikan <command> -o url     # just the URL column
jikan <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
jikan <command> --fields id,title,url
```
...
```bash
jikan <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
jikan <command>            # a table, because this is a terminal
jikan <command> | wc -l    # JSONL, because this is a pipe
```
