// Package cli assembles the jikan command tree from the jikan
...
// NewApp assembles the kit application from the jikan domain. The
...
//
// To add a command, declare it in jikan/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := jikan.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(jikan.Domain{}).Register(app)
